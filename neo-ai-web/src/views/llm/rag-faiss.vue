...
<template>
  <div class="chat-dialog">
    ...
    <div class="chat-input">
      ...
      <a-button type="primary" @click="sendMessage" :disabled="isAssistantResponding">{{ isAssistantResponding ? '回复中...' : '发送' }}</a-button>
    </div>
  </div>
</template>
...
const sendMessage = async () => {
  ...
  messages.value.push({ role: 'assistant', content: '' }); // Add an empty message for streaming
  ...
  try {
    ...
    const raw = JSON.stringify({
      "text": messages.value[messages.value.length - 2].content, // The user's message
    ...
    const response = await fetch(`${baseUrl}/rag_faiss/ask`, requestOptions);
...
const parseChunk = (chunk: string): string => {
  ...
  for (const line of lines) {
    try {
      ...
      if (json.error_msg) {
        createMessage.error(json.error_msg);
        break;
      }
      if (json.result) {
        result += json.result;
